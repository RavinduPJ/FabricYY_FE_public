...
function CreateNew() 
{
    ...
    const [ds_sizetemplate,setds_sizetemplate] = React.useState([]);
    ...
    const [ds_olr_groupbycolor,setds_olr_groupbycolor] = React.useState([]);
    ...
    const [var_sizetempID, setvar_sizetempID] = React.useState("");
    ...
    React.useEffect(()=>{
        ...
        fetch(`${apiurl}/fabricyy/getsizetemplates/${fabyyid}`)
        .then(res_sizestemp => res_sizestemp.json())
        .then(response_sizestemp => 
          {
            if(response_sizestemp.Type === "SUCCESS")
            {
              setds_sizetemplate(response_sizestemp.Data);
        ...
        .then(response_5 => 
          ...
          fetch(`${apiurl}/fabricyy/getolrdatacolorgroup/${fabyyid}`)
        .then(res_olrgroup => res_olrgroup.json())
        .then(response_olrgroup => 
          {
            if(response_olrgroup.Type === "SUCCESS")
            {
              setds_olr_groupbycolor(response_olrgroup.Data);
    ...
    async function RefreshHeaders()
    {
      ...
      async function handleUploadOLR(event) {
        ...
        if(var_sizetempID === "")
        {
          ...
          notification['error']({
            message: 'Data Error',
            description: 'Please Select Size Template',
            style:{color: '#000',border: '1px solid #ff6961',backgroundColor: '#ffa39e'},
          });
          return;
        }
          ...
          var Order_Quantity_cell = Aritzia_worksheet["S1"];
              ...
              else
              {
                  JSON.parse(ExcelToJSON).map(item =>
                  {
                    ...
                    {
                      var orderqtyval = item['Order Quantity'];

                      if(orderqtyval === "" || orderqtyval === 0)
                      {
                        orderqtyval = 0;
                      }
                      else
                      {
                        orderqtyval = Math.round((Number(orderqtyval) / 100) * 104);
                      }
                      
                      OLRITEMS.push({CUSTNAME: var_customer.toUpperCase() ,VPONO: item['Purchasing Document'],DIVISIONCODE:'ARITZIA',TECHPACKNO: item['Vendor Article Number'],MASTSTYLEDESC: item['Short Text'],CUSTSTYLE: item['Vendor Article Number'],CUSTSTYLEDESC: item['Short Text'],MASTCOLORDESC: item['Colour'],MASTSIZEDESC: item['Size'],ORDERQTY: orderqtyval,SEASON: item['Season']})
                ...
                .then(response => { 
                    ...
                    if(response.Type === "SUCCESS")
                    {
                      ...
                      fetch(`${apiurl}/fabricyy/olritemprocess/${fabyyid}/${var_sizetempID}`)
                      .then(res_1 => res_1.json())
                      .then(response_1 => { 
                
                          if(response_1.Type === "SUCCESS")
                          {
                            fetch(`${apiurl}/fabricyy/olrlineprocess/${fabyyid}/${var_sizetempID}`)
      ...
      function handleChangeSizeTemp(value) 
      {
        setvar_sizetempID(value);
      }
      ...
      function createolrgroupbycolortbrow(row_value)
      {
      ...
      function createolrcolorgroupheadrow()
      {
        return(
          <>
          <tr>
      ...
      const handleClickDownload = async () =>{
        ...
        worksheet.getCell(`A${value_plm_row}`).value = "COLOR";
        worksheet.getCell(`A${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
        worksheet.getCell(`B${value_plm_row}`).value = "TOTAL";
        worksheet.getCell(`B${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };

        if(ds_olr_colorset.length > 0){

          worksheet.getCell(`C${value_plm_row}`).value = ds_olr_colorset[0].s1_name;
          worksheet.getCell(`C${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`D${value_plm_row}`).value = ds_olr_colorset[0].s2_name;
          worksheet.getCell(`D${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`E${value_plm_row}`).value = ds_olr_colorset[0].s3_name;
          worksheet.getCell(`E${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`F${value_plm_row}`).value = ds_olr_colorset[0].s4_name;
          worksheet.getCell(`F${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`G${value_plm_row}`).value = ds_olr_colorset[0].s5_name;
          worksheet.getCell(`G${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`H${value_plm_row}`).value = ds_olr_colorset[0].s6_name;
          worksheet.getCell(`H${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`I${value_plm_row}`).value = ds_olr_colorset[0].s7_name;
          worksheet.getCell(`I${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`J${value_plm_row}`).value = ds_olr_colorset[0].s8_name;
          worksheet.getCell(`J${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`K${value_plm_row}`).value = ds_olr_colorset[0].s9_name;
          worksheet.getCell(`K${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`L${value_plm_row}`).value = ds_olr_colorset[0].s10_name;
          worksheet.getCell(`L${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`M${value_plm_row}`).value = ds_olr_colorset[0].s11_name;
          worksheet.getCell(`M${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`N${value_plm_row}`).value = ds_olr_colorset[0].s12_name;
          worksheet.getCell(`N${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`O${value_plm_row}`).value = ds_olr_colorset[0].s13_name;
          worksheet.getCell(`O${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`P${value_plm_row}`).value = ds_olr_colorset[0].s14_name;
          worksheet.getCell(`P${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`Q${value_plm_row}`).value = ds_olr_colorset[0].s15_name;
          worksheet.getCell(`Q${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`R${value_plm_row}`).value = ds_olr_colorset[0].s16_name;
          worksheet.getCell(`R${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`S${value_plm_row}`).value = ds_olr_colorset[0].s17_name;
          worksheet.getCell(`S${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`T${value_plm_row}`).value = ds_olr_colorset[0].s18_name;
          worksheet.getCell(`T${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`U${value_plm_row}`).value = ds_olr_colorset[0].s19_name;
          worksheet.getCell(`U${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`V${value_plm_row}`).value = ds_olr_colorset[0].s20_name;
          worksheet.getCell(`V${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`W${value_plm_row}`).value = ds_olr_colorset[0].s21_name;
          worksheet.getCell(`W${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`X${value_plm_row}`).value = ds_olr_colorset[0].s22_name;
          worksheet.getCell(`X${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`Y${value_plm_row}`).value = ds_olr_colorset[0].s23_name;
          worksheet.getCell(`Y${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`Z${value_plm_row}`).value = ds_olr_colorset[0].s24_name;
          worksheet.getCell(`Z${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`AA${value_plm_row}`).value = ds_olr_colorset[0].s25_name;
          worksheet.getCell(`AA${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`AB${value_plm_row}`).value = ds_olr_colorset[0].s26_name;
          worksheet.getCell(`AB${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`AC${value_plm_row}`).value = ds_olr_colorset[0].s26_name;
          worksheet.getCell(`AC${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`AD${value_plm_row}`).value = ds_olr_colorset[0].s28_name;
          worksheet.getCell(`AD${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`AE${value_plm_row}`).value = ds_olr_colorset[0].s29_name;
          worksheet.getCell(`AE${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
          worksheet.getCell(`AF${value_plm_row}`).value = ds_olr_colorset[0].s30_name;
          worksheet.getCell(`AF${value_plm_row}`).alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
  
        }

        ds_olr_groupbycolor.map((row) => {
          
          value_plm_row = value_plm_row + 1;
          worksheet.getCell(`A${value_plm_row}`).value = row.color;
          worksheet.getCell(`B${value_plm_row}`).value = row.sub_total;
          worksheet.getCell(`C${value_plm_row}`).value = row.s1_qty;
          worksheet.getCell(`D${value_plm_row}`).value = row.s2_qty;
          worksheet.getCell(`E${value_plm_row}`).value = row.s3_qty;
          worksheet.getCell(`F${value_plm_row}`).value = row.s4_qty;
          worksheet.getCell(`G${value_plm_row}`).value = row.s5_qty;
          worksheet.getCell(`H${value_plm_row}`).value = row.s6_qty;
          worksheet.getCell(`I${value_plm_row}`).value = row.s7_qty;
          worksheet.getCell(`J${value_plm_row}`).value = row.s8_qty;
          worksheet.getCell(`K${value_plm_row}`).value = row.s9_qty;
          worksheet.getCell(`L${value_plm_row}`).value = row.s10_qty;
          worksheet.getCell(`M${value_plm_row}`).value = row.s11_qty;
          worksheet.getCell(`N${value_plm_row}`).value = row.s12_qty;
          worksheet.getCell(`O${value_plm_row}`).value = row.s13_qty;
          worksheet.getCell(`P${value_plm_row}`).value = row.s14_qty;
          worksheet.getCell(`Q${value_plm_row}`).value = row.s15_qty;
          worksheet.getCell(`R${value_plm_row}`).value = row.s16_qty;
          worksheet.getCell(`S${value_plm_row}`).value = row.s17_qty;
          worksheet.getCell(`T${value_plm_row}`).value = row.s18_qty;
          worksheet.getCell(`U${value_plm_row}`).value = row.s19_qty;
          worksheet.getCell(`V${value_plm_row}`).value = row.s20_qty;
          worksheet.getCell(`W${value_plm_row}`).value = row.s21_qty;
          worksheet.getCell(`X${value_plm_row}`).value = row.s22_qty;
          worksheet.getCell(`Y${value_plm_row}`).value = row.s23_qty;
          worksheet.getCell(`Z${value_plm_row}`).value = row.s24_qty;
          worksheet.getCell(`AA${value_plm_row}`).value = row.s25_qty;
          worksheet.getCell(`AB${value_plm_row}`).value = row.s26_qty;
          worksheet.getCell(`AC${value_plm_row}`).value = row.s27_qty;
          worksheet.getCell(`AD${value_plm_row}`).value = row.s28_qty;
          worksheet.getCell(`AE${value_plm_row}`).value = row.s29_qty;
          worksheet.getCell(`AF${value_plm_row}`).value = row.s30_qty;
        ...
        <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
              ...
              <Panel header="Fabric YY Details" key="1">
                <Row>
                  ...
                  <Col span={4}>
                    ...
                    <p style={{color:"red"}}>** Select Size Template</p>

                    <Select style={{ width:"100%" }} onChange={handleChangeSizeTemp}>
                    {
                      ds_sizetemplate.map((row) => <Option value={row.temp_id}>{row.temp_name}</Option>)
                    } 
                    </Select>
                    ...
                    <div id="upload-box">
                      <Input value={""}  type="file" onChange={handleUploadOLR} accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"/>
                      <p style={{color:"purple"}}><small>{inputRef}</small></p>
                    </div>
          ...
          <Row>
            ...
            <table style={{marginLeft:"5px"}}>
              <thead style={{fontSize:"10px",textAlign:"center"}}>
                {
                  createolrcolorgroupheadrow()
                }  
              </thead>
              <tbody style={{fontSize:"10px",textAlign:"center",color:"black"}}>
                {
                  ds_olr_groupbycolor.map((row_tb) => createolrgroupbycolortbrow(row_tb))
                } 
              </tbody>
            </table>
            ...
            <AntdDModal width={"90%"} visible={isOpenPlmBomItems} title="PLM Garmet Colorways in PLM Order" onOk={()=>setisOpenPlmBomItems(false)} onCancel={()=>setisOpenPlmBomItems(false)}>
               <table>
               <thead>
                 <tr style={{fontWeight:"bold"}}>
                  ...
                  <td style={{padding:"5px"}}>GMT Color</td>
                  <td style={{padding:"5px"}}>RM Color</td>
                  ...
                  ds_plmbomitems.map((row) => {
                    return(<>
                        <tr>
                          ...
                          <td style={{padding:"5px"}}>{row.gmt_color}</td>
